...
#     weights=None,input_shape=(384, 384, 3),classes = 30
# ) 
def create_feature_extractor(architecture, input_size, weights = None):
    ...
    elif architecture == 'EfficientNetV2S_75':
        feature_extractor = EfficientNetV2SFeature_75(input_size, weights)
...
class EfficientNetV2SFeature(BaseFeatureExtractor):
    ...
    def normalize(self, image):
        ...
        return image
class EfficientNetV2SFeature_75(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size, weights):
        input_image = Input(shape=(input_size[0], input_size[1], 3))
        input_shapes_imagenet = [(384, 384,3),(448, 448,3)]
        ...
        #         mobilenet.load_weights(weights)
        # EfficientNet = keras_efficientnet_v2.EfficientNetV2S( pretrained=None,input_shape=input_shape,include_top=False, rescale_mode="tf")
        EfficientNet = EfficientNetV2S(weights=None,input_shape=input_shape,include_top=False,dropout_rate=0.001,width_coefficient = 0.75,depth_coefficient=0.75) 
